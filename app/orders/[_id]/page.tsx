...
import { useOrderDetails } from "@/app/_lib/useOrders"; // Assuming you have the updated ParticularOrderDetailsResponse type
import Image from "next/image"; // To display product images

// Note: Map dynamic import removed as per request.
...
export default function OrderDetailsPage() {
  ...
  // Assuming data.data[0] is the ParticularOrderDetail object
  const orderDetail = data.data[0]; 
  const orderInfo = orderDetail.order_info;
  const address = orderInfo.address;
  const item = orderDetail.item; // The item_info is now directly under 'item' in the ParticularOrderDetail type
      ...
      <section className="min-h-screen bg-gray-50 p-5 md:p-10">
        ...
        <div className="grid lg:grid-cols-3 gap-8 mb-10">
            ...
            <div className="lg:col-span-1 bg-white border border-gray-200 rounded-xl p-6 shadow-md h-full">
                ...
                <DetailRow label="Category" value={item.categories?.length ? item.categories[0] : 'N/A'} />
                <DetailRow label="Color" value={item.colors?.length ? item.colors[0] : 'N/A'} />
                <DetailRow label="Size" value={item.sizes?.length ? item.sizes[0] : 'N/A'} />
            </div>
        </div>
...
// Component to render the tracking history as a timeline
